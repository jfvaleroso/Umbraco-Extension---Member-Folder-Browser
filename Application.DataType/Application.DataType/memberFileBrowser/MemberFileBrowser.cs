...
using umbraco.IO;
using Application.DataType.memberFolderBrowser;
using umbraco.cms.businesslogic.media;
using umbraco.cms.businesslogic.member;
using umbraco.BusinessLogic;

namespace Application.DataType.memberFileBrowser
{
    ...
    public class MemberFileBrowser : WebControl
    {
            ...
            get
            {
                ...
                if (!string.IsNullOrEmpty(Context.Request.QueryString["id"]))
                {
                    ...
                    if (Int32.TryParse(Context.Request.QueryString["id"], out id))
                    {
                        ...
                        var hasRepo= m.getProperty("repositoryId").Value ;
                        if (string.IsNullOrEmpty(hasRepo.ToString()))
                        {
                            MediaType dt = MediaType.GetByAlias("folder");
                            User u = new User(0);
                            Media d = Media.MakeNew(m.LoginName, dt, u, parentId);
                            m.getProperty("repositoryId").Value = d.Id.ToString();
                            hasRepo = d.Id;
                            m.Save();
                        }
                        return Convert.ToInt32(hasRepo);
                    }
        ...
        protected override void CreateChildControls()
        {
            ...
            foreach (var media in breadCrumb)
            {
                if (media.Id == ParentId)
                    ...
                    if (media.Id == -1)
                        ...
                        sb.AppendFormat("<span>Media > </span>");
                    else
                        //sb.AppendFormat("<li><a href='../editMedia.aspx?id={1}'>{0}</a></li>", media.Text, media.Id);
                        sb.AppendFormat("<span>{0} > </span>", media.Text, media.Id);
            }
            sb.Append("</ul>");
            ...
            Page.ClientScript.RegisterStartupScript(typeof(MemberFolderBrowser),
